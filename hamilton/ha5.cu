...
// k=1 is useless actually
template<>
__global__ void ha2<1>(int n, int work, unsigned *part, int *adj, unsigned long long *ret, unsigned long long mod) {
    const int bid = threadIdx.y + blockIdx.x * blockDim.y;
    ret[bid] = 0;
}
...
#define HA4_k(k) case k: ha2<k><<<gridSize, bsz>>>(n, work, part, adj, ret, mod); break;
    HA4_k(1)HA4_k(2)HA4_k(3)HA4_k(4)HA4_k(5)
    ...
    HA4_k(31)HA4_k(32)HA4_k(33)HA4_k(34)
#undef HA4_k
    }
