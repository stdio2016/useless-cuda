...
__global__ void compute_output_size_gpu(
  ...
  for (int i = 0; i < 12; i++) {
    if (unused >> i & 1) {
      int end = __ldg(&imgpos[choose * 12 + i + 1]);
      for (int j = __ldg(&imgpos[choose * 12 + i]); j < end; j++) {
        uint64_t m = __ldg(&imagelist[j]);
        if ((space & m) == m) {
          ans++;
        }
      }
...
int main() {
  ...
  cudaMalloc(&gpu_out, sizeof(int) * 1000000);
  cudaMalloc(&gpu_next, sizeof(int) * 1000000);
  void     *d_temp_storage = NULL;
  size_t   temp_storage_bytes = 0;
  cub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes, gpu_out, gpu_next, 1000000);
  ...
  for (int step = 2; step <= 12; step++) {
    compute_output_size_gpu<<<4000,256>>>(gpu_imglist, gpu_imgpos,
      gpu_space1, gpu_unused1, ans, gpu_out);
    cub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes, gpu_out, gpu_next, ans);
    int last_size = 0;
    int total_size = 0;
    cudaMemcpy(&last_size, &gpu_out[ans-1], sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(&total_size, &gpu_next[ans-1], sizeof(int), cudaMemcpyDeviceToHost);
    solve_gpu<<<4000,256>>>(gpu_imglist, gpu_imgpos,
      gpu_space1, gpu_unused1, gpu_space2, gpu_unused2, ans, gpu_next);
    ans = last_size + total_size;
