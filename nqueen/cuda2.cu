...
uint *gpu_canplace;
std::vector<long long> cpu_steps;

struct STEP {
  long long s[12];
};

STEP &operator+=(STEP &a, STEP b) {
  for (int i = 0; i < 12; i++) a.s[i] += b.s[i];
  return a;
}
...
__global__ void nqueen_kern(int lv, uint canplace[], struct SubP *works, int workCount,
  ...
  __shared__ uint steps[12][256];
  int tid = threadIdx.x;
  int bid = blockIdx.x, bsize = blockDim.x;
  if (bsize != 256) return;
  int gid = tid + bid * bsize;
  if (gid >= workCount) {
    for (int j = 0; j < 12; j++) result[gid*12+j] = 0;
    return;
  }
  for (int j = 0; j < 12; j++) steps[j][tid] = 0;
  ...
  for (int j = 1; j < 12; j++) result[gid*12+j] = steps[j][tid];
  result[gid*12] = sol;
}
...
STEP nqueen_wait_compute(int nblocks) {
  long long *result = new long long[nblocks*256*12];
  STEP sum = {0};

  cudaDeviceSynchronize();
  cudaMemcpy(result, gpu_result, nblocks*256*12 * sizeof(long long), cudaMemcpyDeviceToHost);

  for (int i = 0; i < nblocks*256; i++) {
    for (int j = 0; j < 12; j++)
      sum.s[j] += result[i*12+j];
  }
...
STEP nqueen_cuda(int n, unsigned canplace[], int nblocks, int nworks) {
  std::vector<SubP> works;
  works.reserve(10000*32);
  cpu_steps = std::vector<long long>(n);
  // create subproblems
  int cut = n - 5;
  if (cut < 2) cut = 2;
  if (cut > 12) cut = 12;
...
int main(int argc, char *argv[]) {
  ...
  cudaMalloc(&gpu_result, sizeof(STEP) * nblocks * 256);
  ...
  while (fscanf(filein, "%d", &n) == 1) {
    ...
    STEP ans = nqueen_cuda(n, canplace, nblocks, gpu_workCount);
    printf("Case #%d:\n", T);
    printf("   0 1\n");
    for (int i = n-1; i >= 12; i--)
      printf("  %2d %lld\n", n-i, cpu_steps[i]);
    for (int i = min(n-1, 11); i >= 0; i--)
      printf("  %2d %lld\n", n-i, ans.s[i] + cpu_steps[i]);
  }
  return 0;
}
